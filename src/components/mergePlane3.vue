...
export default {
  ...
  methods: {
    ...
    initPIXI () {
      ...
      this.tween1.start()
      let maskCount =0, shineCount = 0, that = this
      this.app1.ticker.add(delta => {
        PIXI.tweenManager.update()
        // console.log(that.changeFlag)
        if (that.changeFlag) { //变化
          shineCount += 50 / 3 * delta
          console.log(shineCount)
          if (shineCount >=  that.timeNuit) {
            maskCount += 88 / 3
            that.total -= 1
            maskCount = maskCount > 88 ? 88 : maskCount
            that.updateBlmask(88 - maskCount)
          }
          if (shineCount >= that.timeNuit * 3) {
            that.cycleFuc()
            shineCount = 0
            maskCount = 88
            this.total = 3
